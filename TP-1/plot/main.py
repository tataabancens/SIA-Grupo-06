...
def plot_by_method(data: json, title: str, y_label: str, mapper, filter_func=None):
    ...
    for stats in data:
        ...
        if idx >= len(processed[method]):
            processed[method].extend(
                [None] * (idx - len(processed[method]) + 1))
...
def plot_lines(data: json, title: str, x_label: str, y_label: str, x_mapper, y_mapper, filter_func=None):
    ...
    for (method, values) in processed.items():
        xs = [v for (idx, v) in enumerate(x_values) if idx <
              len(values) and values[idx] is not None]
...
def main():
    ...
    if len(os.sys.argv) < 1:
        print("Usage: python main.py <data-<date>.json>")
        exit(1)

    with open(f"{os.sys.argv[1]}", 'r', encoding="utf-8") as json_file:
        data = json.load(json_file)
    plot_by_method(data, "Tiempo / método de búsqueda", "tiempo (seg.)",
                   lambda v: v['elapsed_time'], lambda v: v['agents'] == 3 and v['grid_size'] <= 8)
    plot_by_method(data, "Nodos frontera / método de búsqueda", "nodos frontera",
                   lambda v: v['frontier_nodes'], lambda v: v['agents'] == 3)
    plot_by_method(data, "Nodos explorados / método de búsqueda", "nodos explorados",
                   lambda v: v['nodes_explored'], lambda v: v['agents'] == 1)
    plot_by_method(data, "Costo total / método de búsqueda",
                   "costo", lambda v: v['cost'])
    # filter so the number of agents stays constant and the only variable is grid size
    plot_lines(data, "Nodos explorados / largo de grilla", "largo de grilla", "nodos explorados",
               lambda v: v['grid_size'], lambda v: v['nodes_explored'], lambda v: v['agents'] == 3)
    # filter so the number of agents stays constant and the only variable is grid size
    plot_lines(data, "Tiempo / largo de grilla", "largo de grilla", "Tiempo",
               lambda v: v['grid_size'], lambda v: v['elapsed_time'], lambda v: True)
    plot_lines(data, "Costo / nodos explorados", "nodos explorados", "costo",
               lambda v: v['nodes_explored'], lambda v: v['cost'], lambda v: True)
