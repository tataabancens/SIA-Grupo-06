...
class LinealPerceptron(SimplePerceptron):
    ...
    def save_data(self, current_epoch, error, **kwargs):
        ...
        try:
            ...
            pass
        self.change = False
        self.data.save_data(weights=self.weights, error=error, current_epoch=current_epoch, gen_error=gen_error)
    ...
    def train_and_test(self, data: Dataset, epoch_limit: int, dataset_divider):
        ...
        while min_error > self.get_tolerance() and current_epoch < epoch_limit:
            current_input, expected_output = self.random_np_input_from_list(train_data.inputs, train_data.outputs)
    ...
    def compute_error(self, data: list[list[float]], expected_outputs: list[float]):
        ...
        for index, cur in enumerate(data):
            ...
            inverse_scaled_act = self.inverse_scale(activation, self.dataset.min_output, self.dataset.max_output)
            inverse_scaled_cur = self.inverse_scale(current_output, self.dataset.min_output, self.dataset.max_output)
